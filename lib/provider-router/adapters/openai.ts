/**
 * OpenAI adapter — wraps existing OpenAIProvider (sole SDK boundary).
 */
...
function messagesToParts(messages: PromptMessage[]): {
  ...
  const prompt = messages
    ...
    .join("\n\n");
  return { systemInstruction: system, prompt: prompt || messages.at(-1)?.content || "" };
}

export function createOpenAIAdapter(
  providerInstance?: OpenAIProvider
): ProviderAdapter {
  const provider = providerInstance ?? new OpenAIProvider();
  return {
    id: "openai" as ProviderId,
    isConfigured() {
      return isProviderKeyPresent("openai");
    },
